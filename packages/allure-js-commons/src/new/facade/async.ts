import { AsyncTestRuntime, getGlobalTestRuntime } from "../TestRuntime.js";
...
export const label = (name: LabelName, value: string) => {
  const runtime = getGlobalTestRuntime<AsyncTestRuntime>();
...
export const step = (name: string, body: (context: StepContext) => Promise<void>) => {
  const runtime = getGlobalTestRuntime<AsyncTestRuntime>();
