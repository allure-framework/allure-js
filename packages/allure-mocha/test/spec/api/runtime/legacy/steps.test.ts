...
describe("step", () => {
  ...
  it("may contain an attachment", () => {
    const [substep] = testMap.get("a step with an attachment")?.steps[0]?.steps ?? [];

    expect(substep.name).toEqual("foo.txt");
    expect(substep.attachments).toEqual([
      expect.objectContaining({
        name: "foo.txt",
        type: "text/plain",
      }),
    ]);
    const source = substep.attachments[0].source;
