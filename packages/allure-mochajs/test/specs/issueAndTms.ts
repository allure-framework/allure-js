...
import { cleanResults, findLabel, findLinks, findTest, runTests, whenResultsAppeared } from '../utils/index';
...
class IssueAndTmsSuite {
  ...
  shouldHaveIssueAndTmsLinks() {
    ...
    return whenResultsAppeared().then(results => {
      ...
      expect(links.map(link => link.url)).contains('http://localhost/issues/1', 'http://localhost/issues/2');
